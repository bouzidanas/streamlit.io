		...
		init: function( reveal ) {
			...
			if( !renderer ) {
				...
				renderer.code = ( code, language ) => {
					...
					// Off by default
					let lineNumbers = '';
                    let codeBlock = `<pre><code ${lineNumbers} class="${language}">${code}</code></pre>`;
					...
					if( CODE_LINE_NUMBER_REGEX.test( language ) ) {
                        ...
                        codeBlock = `<pre class="code-wrapper">`;
                        lineNumbers = `data-line-numbers="${lineNumbers}"`;
                        language = language.replace( CODE_LINE_NUMBER_REGEX, '' ).trim();

                        codeBlock += `<code ${lineNumbers} class="${language} has-highlights">${code}</code></pre>`;
					}
